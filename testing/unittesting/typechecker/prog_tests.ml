...
let test_typecheck_decl_of_var_in_fun_body_with_same_name_as_other_function ctxt =
  let ast = mk_prog ([
  ]) (Sglobal_list []) 
    (Sfundec_list 
    [ (mk_stmt 
        (Sfunc 
          (mk_fundec 
            ...
            [] 
            (Slist 
              [ (mk_stmt (Sreturn (mk_expr (EConst 1))))]
            )
          )
        )
      );
      (mk_stmt 
        (Sfunc 
          (mk_fundec 
            ...
            [] 
            (Slist 
              [(mk_stmt (Sdecl { var_ty = Int_ty; var_name = mk_ident "f"; var_expr = None }))
              ;(mk_stmt (Sreturn (mk_expr (EConst 1)))) 
              ]
            )
          )
...
let test_typecheck_same_var_declared_in_2_functions ctxt =
  let ast = mk_prog ([
  ]) (Sglobal_list []) 
    (Sfundec_list 
    [ (mk_stmt 
        (Sfunc 
          (mk_fundec 
            ...
            [] 
            (Slist 
              [ (mk_stmt (Sdecl { var_ty = Int_ty; var_name = mk_ident "x"; var_expr = None }))
              ; (mk_stmt (Sreturn (mk_expr (EConst 1))))]
            )
          )
        )
  ...
  let typecheck = Typechecker.program ast in 
  let expected_out = "INTf(){letINTxreturn(INT1)}INTg(){letINTxreturn(INT1)}" in
  let ttree_string = remove_whitespace (Pp_type.pp_prog typecheck) in
  assert_equal  ~printer:(fun x -> x) expected_out ttree_string


let test_typechecker_call_of_func2_from_func1  ctxt =
let ast = mk_prog ([
  ]) (Sglobal_list []) 
    (Sfundec_list 
    [ (mk_stmt 
        (Sfunc 
          (mk_fundec 
            ...
            [] 
            (Slist 
              [(mk_stmt (Sreturn (mk_expr (EFcall(mk_ident "g", [])))))]
            )
          )
        )
      );
  ...
  let typecheck = fun () -> let _ = Typechecker.program ast in () in
  assert_raises (mk_error "Function g has not been declared") typecheck
  ...
  let prog_tests = "Program typechecking tests" >::: [
    ...
    "Decl of var in fun body with same name as other function" >:: test_typecheck_decl_of_var_in_fun_body_with_same_name_as_other_function;
    "Same var declared in 2 functions" >:: test_typecheck_same_var_declared_in_2_functions;
    "Call of func2 from func1" >:: test_typechecker_call_of_func2_from_func1;
  ]  
