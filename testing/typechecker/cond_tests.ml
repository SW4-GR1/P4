...
let test_typechecker_cond_eq_integers test_ctxt =
  ...
  assert_equal Ttree.Tbool ty  (* Expect the type of an equality expression to be boolean *)
...
let test_typechecker_cond_neq_bools test_ctxt =
  ...
  let vtab = mk_vtab in
  let (ty, expr) = Typechecker.checkExp ftab vtab ast in
  assert_equal Ttree.Tbool ty  (* Expect the type of an inequality expression to be boolean *)
